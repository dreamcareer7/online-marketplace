class City < ApplicationRecord
  ...
  def cached_city_banners
    Rails.cache.fetch("#{Rails.env}_cachedAllCityBanners_#{id}_#{I18n.locale}") {
      city_banners.to_a
    }
  end
  ...
  def city_image
    ...
    self.cached_city_banners.sample
  end
  ...
  def clear_cache
    I18n.available_locales.each do |locale|
      Rails.cache.fetch("#{Rails.env}_cachedAllCityBanners_#{id}_#{locale}")
      Rails.cache.delete("#{Rails.env}_get_cached_city_#{self}_#{locale}")
      Rails.cache.delete("#{Rails.env}_popular_items_new_#{id}_#{locale}")
      Rails.cache.delete("#{Rails.env}_popular_by_listing_#{id}_#{locale}")
    end
  end
end
