...
$(document).on('turbolinks:load', function () {
  $(".category_select_item").on("click", function (e) {
    ...
    if (!that.hasClass("category_selected")) {
      $(".category_selected").removeClass("category_selected");
      that.addClass("category_selected");
      var category = that.data("category"),
        project = that.data("project"),
        url = "/user/projects/" + project + "/category_details?category_id=" + category;

     
      $('#category_details_all').html("<div class='loading_img_wrapper'><img src='https://public-assets-muqa.s3-ap-southeast-1.amazonaws.com/loading_main.gif' /></div>");
      $('#category_details_all').load(url + "#card-wrapper", function() {
        $(".product_hidden_category").val(category);
        $('#category_details_all .select2').addClass('needsclick').select2({
        ...
        $('#category_details_all .js-selected-services').on('change', function() {
        ...
        $('#category_details_all .js-selected-subcat').on('change', function() {
  ...
  // Select2
  $('.select2').addClass('needsclick').select2({
    templateSelection: hideNone,
    templateResult: hideNone
  });
  ...
  $('.js-sidebar-expand').click(function () {
    ...
    $(document).find('.js-sidebar-list').each(function () {
      $(this).hide();
    });
    $(document).find('.js-sidebar-expand').each(function () {
      $(this).removeClass('expanded');
    });
